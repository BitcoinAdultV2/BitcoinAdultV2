...
bool CWallet::AddDeterministicSeed(const uint256& seed)
{
    ...
    if(IsCrypted()) {
        ...
        strErr = "save zBTADseed to wallet";
    }
    //the use case for this is no password set seed, mint dzBTAD,
...
{
    //If this is a zapwallettx, need to read zBTAD
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zBTAD output", __func__);
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zBTAD public spend transaction
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to BTAD address to the transaction (the actual primary spend taking place)
                // TODO: check value?
