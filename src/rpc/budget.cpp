...
UniValue preparebudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"BTAD-address\" monthy-payment\n"
            ...
            "5. \"BTAD-address\":   (string, required) BTAD address to send payments to\n"
    ...
    if (!pwalletMain->CreateBudgetFeeTX(wtx, proposalHash, keyChange, false)) { // 50 BTAD collateral for proposal
...
UniValue submitbudget(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"BTAD-address\" monthly-payment \"fee-tx\"\n"
