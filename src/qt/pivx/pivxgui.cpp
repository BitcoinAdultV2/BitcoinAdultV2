...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Bitcoinadult client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Bitcoinadult"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Bitcoinadult"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
