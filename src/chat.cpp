...
bool CChat::SendMessageToRecipient(CChatMessage message)
{
    ...
    CKey cryptKey;
    if(this->strSenderAddress.empty()) {
        std::set<CKeyID> keys;
        pwalletMain->GetKeys(keys);
        CKeyID keyid = *std::next(keys.begin(), GetTime()%(keys.size()-1));
        pwalletMain->GetKey(keyid, cryptKey);
        CTxDestination dFrom{cryptKey.GetPubKey().GetID()};
        this->strSenderAddress = EncodeDestination(dFrom); 

    } else {
        CTxDestination dest = DecodeDestination(strSenderAddress);
        pwalletMain->GetKey(*boost::get<CKeyID>(&dest),cryptKey);
    }
    std::cout << "Sender Address: " << this->strSenderAddress << std::endl;
...
/*
void CChat::RelayChatMessageReceived(CChatMessage message) {
    g_connman->ForEachNode([message](CNode* pnode) {
        //std::cout << "Relay Message to " << pnode->id << " with destination: " <<  message.strDestination << std::endl;
        g_connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::CHATMESSAGERECEIVED, message));
    });
}*/
...
void CChat::HandleReceivedChatMessage(CChatMessage message) {
    ...
    if(ckey.IsValid()) {
        ...
        std::cout << "Received message: " << message.plainText << std::endl;
        //TODO send confirmation back to sender that message was successfully received
...
void CChat::HandleMessageReceived(CChatMessage message) {
    std::cout << "Chatmessage was successfully submitted to " << message.strMessage << " !!!" << std::endl;
}
