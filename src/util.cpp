...
const char * const PIVX_CONF_FILENAME = "bitcoinadult.conf";
const char * const PIVX_PID_FILENAME = "bitcoinadult.pid";
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bitcoinadultParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bitcoinadultParams
    // Mac: ~/Library/Application Support/bitcoinadultParams
    // Unix: ~/.bitcoinadult-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinadultParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "BitcoinadultParams";
#else
    // Unix
    return pathRet / ".bitcoinadult-params";
#endif
#endif
}
